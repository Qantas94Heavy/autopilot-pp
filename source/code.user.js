...
{	'use strict';	
	...
	{	// default arguments
		...
		// purely informational, no effect if changed (except in debug)
		this._kp = kp;
		this._ki = ki;
		this._kd = kd;
		...
		{   this._setPoint = setPoint;
			...
			{	// yes this is naughty
				...
				// both are placed for backwards compatibility
				this._lastError = this._lastErr
				                = lastError
								= error;
								
				errSum += error * dt;
				var proportional = this._proportional = kp * error;
				var integral = ki * errSum;
				var derivative = this._derivative = kd * dError;
				...
				return this._output = proportional + integral + derivative;
			};
		};
	...
	{	var href = PAGE_PATH
		...
		{	// use setTimeout to make function asynchronous (necessary? - fetchKml is already async)
			setTimeout(function ()
			{	if (kmlObject)
				{	try { ges.aircraft.setup = eval(kmlObject.getDescription())[0]; }
					...
					if (typeof ges.aircraft.setup.airbrakesTravelTime === 'number') ges.aircraft.setup.airbrakesTravelTime = 1; // adjust unrealistic spoiler extension time
					ges.aircraft.setup.brakeDamping = 5;
